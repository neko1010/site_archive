...
def process():
    ...
    if any(os.path.splitext(f)[1] == '.xml' and os.path.split(f)[1].startswith("SV") for f in files):
        try:
            ...
            dup_root = Tk()
            dup_root.withdraw()
            messagebox.showerror("ERROR", "File may already exist")
            dup_root.destroy()
...
explanation = "Select site visit files and archive them with the click of a button! This version requires a 'master' .xml document that will guide ancillary files associated with the site visit to the proper location. Only one site visit and associated supporting documents are currently supported."
